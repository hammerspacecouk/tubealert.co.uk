...
test("getAllLatest returns result", () => {
  ...
  // setup and run model
  const model = new Status(mockDocumentClient, defMockDateTimeHelper, defMockLineModel, defMockConfig, defMockLogger);
  return model.getAllLatest("now").then(d => {
    // assertions
    expect(mockQueryFunction.mock.calls[0][0].ExpressionAttributeValues[":date"]).toBe("tube-date");
    expect(mockQueryFunction.mock.calls[0][0].ScanIndexForward).toBe(false);
...
test("getLatestDisrupted", () => {
  ...
  // setup and run model
  const model = new Status(mockDocumentClient, defMockDateTimeHelper, defMockLineModel, defMockConfig, defMockLogger);
  return model.getLatestDisrupted("now").then(result => {
...
test("fetch and store", () => {
  ...
  const expectedData = [
    ...
    },
    {
      ...
      latestStatus: {
        ...
        title: "D, C, B",
        shortTitle: "D, C",
        descriptions: ["NOTOK-D1", "NOTOK-D2", "OK-B"],
      },
    },
  ];
  ...
  const model = new Status(mockDocumentClient, defMockDateTimeHelper, mockLineModel, defMockConfig, defMockLogger);

  return model.fetchNewLatest().then(result => {
