...
WebPush.setVapidDetails(`mailto:${process.env.CONTACT_EMAIL}`, process.env.PUBLIC_KEY, process.env.PRIVATE_KEY);
...
const getNotificationModel = () =>
  new Notification(documentClient, WebPush, assetManifest, getDateTimeHelper(), BatchWriteHelper, config, console);
const getStatusModel = () => new Status(documentClient, getDateTimeHelper(), getLineModel(), config, console);
const getSubscriptionModel = () => new Subscription(documentClient, BatchWriteHelper, TimeSlotsHelper, console);
...
const getStatusController = callback =>
  new StatusController(callback, getDateTimeHelper(), getStatusModel(), JsonResponseHelper, console);

const getSubscriptionsController = callback =>
  new SubscriptionsController(callback, getSubscriptionModel(), getDateTimeHelper(), JsonResponseHelper, console);

const getWebappController = callback =>
