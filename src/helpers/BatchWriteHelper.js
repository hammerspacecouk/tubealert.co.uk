...
class BatchWriteHelper {
    ...
    makeRequests(requests, total) {
        ...
        const remaining = requests.slice(MAX_BATCH_SIZE);
        ...
        return this.documentClient.batchWrite(params).promise()
            .then(data => {
                ...
                if ("UnprocessedItems" in data && this.tableName in data.UnprocessedItems) {
                    remaining.push(data.UnprocessedItems[this.tableName]);
                }
                return remaining;
            })
            .then(remaining => this.makeRequests(remaining, total));
    }
}
